# code from https://github.com/harvard-edge/multilingual_kws
...
import numpy as np
import func.input_data as input_data
...
class StreamingAccuracyStats(object):
  ...
  def __init__(self, target_keyword):
    ...
    self._which_matched = {}
    self._which_wrong = {}
    # these are for single-target classifiers, wont work for multikeyword datasets
    self._how_many_gt_target = 0
    self._how_many_gt_unknown_or_silence = 0
  ...
  def calculate_accuracy_stats(self, found_words, up_to_time_ms,
    ...
    for ground_truth in self._gt_occurrence:
      ...
      if (ground_truth_label == input_data.SILENCE_LABEL 
          or ground_truth_label == input_data.UNKNOWN_WORD_LABEL):
        self._how_many_gt_unknown_or_silence += 1
      elif ground_truth_label == self.target_keyword:
        self._how_many_gt_target += 1
    ...
    # mmaz
    self._how_many_fn = 0
    words = [input_data.SILENCE_LABEL, input_data.UNKNOWN_WORD_LABEL, self.target_keyword]
    self._which_matched = { w:0 for w in words}
    self._which_wrong = { w:0 for w in words}
    ...
    for found_word in found_words:
      ...
      for ground_truth in self._gt_occurrence:
        ...
        has_gt_matched.add(ground_truth_time)

        # TODO(mmaz)
        # sgt = set(has_gt_matched)
        # if len(has_gt_matched) > len(sgt):
        #   print("\n\nERROR!!! DUPLICATE GT MATCHES!!! {}\n\n".format(len(has_gt_matched) - len(sgt)))
    ...
    # mmaz: count False Negatives
    gt_so_far = filter(lambda o: o[1] < latest_possible_time, self._gt_occurrence)
    for ground_truth in gt_so_far:
      ground_truth_time = ground_truth[1]
      ground_truth_label = ground_truth[0]
      ...
      for found_word in found_words:
        ...
        if (ground_truth_time < latest_time and ground_truth_time > earliest_time):
          # do not count found_label != ground_truth_label for false negatives, 
          # only count when no detections occurred
          missed = False
  ...
  def print_accuracy_stats(self):
    ...
    if self._how_many_gt == 0:
      ...
      #     self._how_many_fp))
      print('No ground truth yet, {}false positives'.format(
          self._how_many_fp))
    else:
      ...
      # mmaz:
      false_negative_percentage = self._how_many_fn / self._how_many_gt * 100
      #print("#GT", self._how_many_gt)
      info = (
          '{:.1f}% matched, {:.1f}% correct, {:.1f}% wrong, '
          '{:.1f}% false positive, {:.1f}% false negative, '
          '{:.1f} howmanyfp, {:.1f} howmanyfn'.format(any_match_percentage,
                                          correct_match_percentage,
                                          wrong_match_percentage,
                                          false_positive_percentage,
      ...
      print(info)
      stat = {
        "correct_match_percentage": correct_match_percentage,
        "wrong_match_percentage": wrong_match_percentage,
        "howmanyfp": self._how_many_fp,
        "howmanyfn": self._how_many_fn,
        "wrong": self._which_wrong.copy(),
        "matched": self._which_matched.copy(),
        "num_groundtruth_target": self._how_many_gt_target,
        "num_groundtruth_unknown_or_silence": self._how_many_gt_unknown_or_silence,
      }
      return info, stat