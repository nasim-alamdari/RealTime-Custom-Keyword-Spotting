...
def transfer_learn(
    ...
    # Randomly SpecAugment for training and validation dataset
    # About autotune: tf. data builds a performance model of the input pipeline and runs an optimization algorithm
    ...
    """
    tf.Dataset.shuffle().repeat().barch(): 
    ...
    Because the repeat is after the shuffle, batches will be always different, even across epochs.
    """
    train_ds = init_train_ds.shuffle(buffer_size=1000).repeat().batch(batch_size)
    #val_ds   = init_val_ds.shuffle(buffer_size=100).repeat().batch(batch_size)
    val_ds = init_val_ds.batch(batch_size)
    
    # Nasim debuggin ======================================
    shape_train_init = init_train_ds.cardinality().numpy()
    shape_valid_init = init_val_ds.cardinality().numpy()
    print("shape_train_init:", shape_train_init)
    print("shape_valid_init:", shape_valid_init)
    
    """shape_train_batch = train_ds.cardinality().numpy()
    shape_valid_batch = val_ds.cardinality().numpy()
    print("shape_train_batch:", shape_train_batch)
    print("shape_valid_batch:", shape_valid_batch)
    
    cnt = 0
    for x, y in train_ds:
        cnt+=1
        if cnt <=2:
            print("train labels in one batch:", y)  
    #print("train cnt: ", cnt)
    ...
    for x, y in val_ds:
        cnt+=1
        print(y)   
    print("validation cnt:", cnt)"""

    
    # End of Nasim debuggin ===============================
