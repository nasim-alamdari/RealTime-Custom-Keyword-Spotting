...
class AudioDataset:
    ...
    def augment(self, audio, label): # Randomly do one of these 3 options: 1) generate background noise,  or 2) do time shift to unknow file, or 3) time shift and add noise to target keyword audio file
    ...
    def spec_augment(self, spectrogram): # perform specAugment on a given spectrogram (spectrogram can be background noise, target audio, or unknow audio )
        # https://git.io/JLvGB
    ...
    def _add_bg(self, audio): # add background noise
        background_volume = self.gen.uniform([], 0, self.background_volume_range)
        background_audio = self.random_background_sample()
    ...
    def init_single_target(self, AUTOTUNE, files, is_training): 
        ...
        # Randomly do one of these 3 options: 1) generate background noise,  or 2) do time shift to unknow file, or 3) add noise to target keyword audio file
        waveform_ds = (
